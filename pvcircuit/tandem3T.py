...
class Tandem3T(object): 
    ...
    def VI0(self, VIname, meastype='CZ'):
        ...
        if VIname == 'VztIro':
            ...
            tmptop.set(name='tmptop', Rser = top.Rser + self.Rz / self.totalarea * top.totalarea , JLC=0.) # correct Rz by area ratio
    ...
    def controls(self, Vdata3T=None, Idata3T=None, darkData3T=None, hex=False, meastype='CZ', size='x-large',
        ...
        def on_3Treplot(change):
            ...
            if VdataMPP:
                outstr = (fmtstr+',   Pmp = {6:>5.2f} mW/cm2').format(VdataMPP.Vzt[0], 
                        VdataMPP.Vrz[0],VdataMPP.Vtr[0],
                        VdataMPP.Iro[0]*fscale, VdataMPP.Izo[0]*fscale, 
                        VdataMPP.Ito[0]*fscale, VdataMPP.Ptot[0]*fscale)
                with VoutBox:   print(outstr.replace('Fit:','VData:'))

            elif IdataMPP:
                outstr = (fmtstr+',   Pmp = {6:>5.2f} mW/cm2').format(IdataMPP.Vzt[0], 
                        IdataMPP.Vrz[0],IdataMPP.Vtr[0],
                        IdataMPP.Iro[0]*fscale, IdataMPP.Izo[0]*fscale, 
                        IdataMPP.Ito[0]*fscale, IdataMPP.Ptot[0]*fscale)           
                with VoutBox:   print(outstr.replace('Fit:','IData:'))
            ...
            if 'MPP' in fitsp.names: # not fast
                ...
                outstr = fmtstr.format(fitsp.Vzt[0], fitsp.Vrz[0],fitsp.Vtr[0],
                        fitsp.Iro[1]*fscale, fitsp.Izo[1]*fscale, fitsp.Ito[1]*fscale,
                        fitsp.Ptot[ii]*fscale)
                with VoutBox:   print(outstr.replace('fit','data'))
            else:
                outstr = fmtstr.format(fitsp.Vzt[0], fitsp.Vrz[0],fitsp.Vtr[0],
                        fitsp.Iro[1]*fscale, fitsp.Izo[1]*fscale, fitsp.Ito[1]*fscale)
                with VoutBox:   print(outstr.replace('fit','data'))
            ...
            if Vcalc:
                ...
                with Rout: Ifit3T.plot(inplot = (Iax, Iobjs), cmap=None, ccont='red', **Iargs)  #replot fit contour
            ...
            if Icalc:
                ...
                with Lout: Vfit3T.plot(inplot = (Vax, Vobjs), cmap=None, ccont='red', **Vargs) #replot fit contour
